...
// LOCAL PATCHES on top of the vendored ui.8starlabs.com/r/flip-clock.json
// registry source — re-running `shadcn add` for this component will clobber
// all of the below, so re-apply by hand if that ever happens:
//   - added the `now?: Date` prop (getTime's third param) so a caller can
//     drive real-time mode with any instant, not just `new Date()`.
//   - lazy-init `time` state from `getTime(...)` instead of a hardcoded
//     all-zero TimeLeft, so SSR/first-paint markup shows the real value
//     instead of "00:00:00".
//   - `type`-only imports for VariantProps/FC/HTMLAttributes/ReactNode
//     (this project's bundler and `verbatimModuleSyntax` both require it;
//     the upstream file imports them as regular values).
//   - reduced-motion guard in FlipUnit (skip ever setting `flipping`).
//   - `<style jsx global>` -> plain `<style>` (styled-jsx is Next.js-only;
//     this stack doesn't run that transform).
//   - `aria-live="polite"` -> `"off"` on the sr-only live region. The
//     upstream value re-announces the full time to screen readers on every
//     second's re-render, which makes a page with this clock continuously
...
const FlipUnit: FC<FlipUnitProps> = memo(function FlipUnit({
  ...
  useEffect(() => {
    if (digit !== prevDigit) {
      // The flip keyframes are raw CSS injected below (not Tailwind
      // utilities), so a `motion-reduce:` variant can't reliably beat them
      // in the cascade. Skip ever setting `flipping` instead: under
      // reduced motion the digit swaps straight to its new value with no
      // animated flap in play at all.
...
interface FlipClockProps
  ...
  /**
   * Overrides the instant the clock reads in real-time mode. Defaults to
   * `new Date()`. `getTime()` only ever reads this Date through its local
   * getters (getHours/getMinutes/getSeconds), so a caller can display any
   * timezone's wall clock by constructing a Date whose local fields already
   * equal that timezone's civil time, rather than the visitor's own.
   */
  now?: Date;
}
...
export default function FlipClock({
  ...
  // Lazy-initialized from the real clock: the effect below never runs
  // during SSR, so a hardcoded zero default would paint "00:00:00" into
  // the server-rendered markup (and flash it again pre-hydration).
  const [time, setTime] = useState<TimeLeft>(() => getTime(countdown, targetDate, now));

  useEffect(() => {
    setTime(getTime(countdown, targetDate, now));

    // Run a faster heartbeat (250ms) to catch the second rollover immediately
    const timer = setInterval(() => {
      const nextTime = getTime(countdown, targetDate, now);
  ...
  return (
    ...
    >
      ...
      {/* Plain <style>, not styled-jsx: `jsx`/`global` are a Next.js-only
          transform this Astro+Vite stack doesn't run, so they'd otherwise
          just be inert (and React would warn about invalid attributes on
          a DOM <style> element). */}
      <style>{`
...
function getTime(countdown: boolean, targetDate?: Date, now: Date = new Date()): TimeLeft {
